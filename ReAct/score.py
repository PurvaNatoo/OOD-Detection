...
def get_react_score(inputs, model, forward_func, method_args, logits=None):
    if logits is None:
        with torch.no_grad():
            logits = forward_func(inputs, model)
...
def get_odin_score(inputs, model, forward_func, method_args):
    ...
    labels = torch.autograd.Variable(torch.LongTensor(maxIndexTemp))
...
def get_score(inputs, model, forward_func, method, method_args, logits=None):
    if method == "msp":
        ...
        scores = get_react_score(inputs, model, forward_func, method_args, logits)
    return scores
