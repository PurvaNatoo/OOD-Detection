...
def sample_estimator(model, num_classes, feature_list, train_loader):
    ...
    for data, target in train_loader:
        ...
        for i in range(num_output):
            ...
            out_features[i] = torch.mean(torch.tensor(out_features[i].data), dim=2)
    ...
    for num_feature in feature_list:
        ...
        for j in range(num_classes):
            print("mean ", list_features[out_count][j])
            fea_value = list_features[out_count][j]
            temp_list[j] = torch.mean(torch.tensor(fea_value), dtype=float)
    ...
    for k in range(num_output):
        ...
        for i in range(num_classes):
            if i == 0:
                ...
                print(X)
                print(X.shape)
                Y = list_features[k][i] - sample_class_mean[k][i]
                print("old Y", Y.shape)
                print(Y)
        ...
        # find inverse
        group_lasso.fit(X.reshape(-1,1).cpu().numpy())
        temp_precision = group_lasso.precision_
        temp_precision = torch.from_numpy(temp_precision).double()
