...
def get_model(args, num_classes, load_ckpt=True):
    if args.in_dataset == 'imagenet':
        ...
        if load_ckpt:
            checkpoint = torch.load("./checkpoints/{in_dataset}/{model_arch}/checkpoint_{epochs}.pth.tar".format(in_dataset=args.in_dataset, model_arch=args.name, epochs=args.epochs), map_location=torch.device('cpu'))
