...
'''
def normalizer(x):
   if np.linalg.norm(x, axis=-1, keepdims=True) != 0:
       v = x / np.linalg.norm(x, axis=-1, keepdims=True) +  1e-10
   else:
       v = 0
   return v
''' 
...
for K in range(1, 10):
    print("-3" + str(K))
    rand_ind = np.random.choice(id_train_size, int(2048 * ALPHA), replace=False)
    print(str(rand_ind))
    ...
    print("here")
    index2.add_with_ids(ftrain, rand_ind)
    ###end check
    '''
    
    print(str(ftrain.shape))
