...
dummy_input = torch.zeros((1, 3, 32, 32)).to(device=device)
...
for split, in_loader in [('train', trainloaderIn), ('val', testloaderIn),]:
    ...
    if FORCE_RUN or not os.path.exists(cache_name):
        ...
        x = np.array((feat_log.T,score_log.T,label_log),dtype=object)
        #np.save(cache_name, (feat_log.T, score_log.T, label_log))
        np.save(cache_name, x)
    else:
...
for ood_dataset in args.out_datasets:
    ...
    if FORCE_RUN or not os.path.exists(cache_name):
        ...
        y = np.array((ood_feat_log.T, ood_score_log.T),dtype=object)
        #np.save(cache_name, (ood_feat_log.T, ood_score_log.T))
        np.save(cache_name, y)
    else:
